...
// camera. Pass the viewport width and height. Returns false if the world point is behind the camera
bool CCamera::PixelFromWorldPt(CVector2* PixelPt, CVector3 worldPt, TUInt32 ViewportWidth, TUInt32 ViewportHeight)
{
	CVector4 P = { worldPt.x, worldPt.y, worldPt.z, 1.0f };
...
/*bool CCamera::PixelFromWorldPt( CVector3 worldPt, TUInt32 ViewportWidth, TUInt32 ViewportHeight,
                                TInt32* X, TInt32* Y )
{
	CVector3 vpPoint = m_MatViewProj.TransformPoint(worldPt);
	if (vpPoint.z < 0)
	{
		return false;
	}

	vpPoint.x = vpPoint.x / vpPoint.z;
	vpPoint.y = vpPoint.y / vpPoint.z;

	*X = static_cast<TInt32>((vpPoint.x + 1.0f) * ViewportWidth * 0.5f);
	*Y = static_cast<TInt32>((1.0f - vpPoint.y) * ViewportHeight * 0.5f);
...
CVector3 CCamera::WorldPtFromPixel(CVector2 pixelPt, TUInt32 ViewportWidth, TUInt32 ViewportHeight)
{
	...
	CVector3 worldPoint = (Q * Inverse(m_MatViewProj)).Vector3();
